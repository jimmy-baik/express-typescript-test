...
export const feedsTable = sqliteTable("feeds", {
    ...
    slug: text('slug').notNull().unique(), // url에 노출될 피드의 고유 주소
    title: text('title').notNull(), // 사용자가 지정한 피드 제목
    createdAt: int('created_at', { mode: 'timestamp' }).notNull(), // sqlite는 timestamp 타입이 없으므로 int로 저장
...
export const postsTable = sqliteTable("posts", {
    ...
    createdAt: int('created_at', { mode: 'timestamp' }).notNull(), // sqlite는 timestamp 타입이 없으므로 unix timestamp int로 저장
    originalUrl: text('original_url').notNull(), // 컨텐츠 원본 URL
    textContent: text('text_content').notNull(), // 불필요한 요소를 제거한 컨텐츠. 컨텐츠 요약과 임베딩 생성에 사용
    htmlContent: text('html_content'), // html formatted 된 컨텐츠. optional하게 캐시된 버전의 컨텐츠를 보여줘야 할 경우에만 사용
    title: text('title'), // 컨텐츠 제목
    generatedSummary: text('generated_summary'), // 자동 생성된 요약
    embedding: text('embedding'), // JSON string으로 저장, nullable
});
...
export const feedInvitesTable = sqliteTable("feed_invites", {
    ...
    createdAt: int('created_at', { mode: 'timestamp' }).notNull(), // sqlite는 timestamp 타입이 없으므로 int로 저장s
    expiresAt: int('expires_at', { mode: 'timestamp' }).notNull(), // sqlite는 timestamp 타입이 없으므로 int로 저장
    isActive: int('is_active', { mode: 'boolean' }).notNull().default(true), // 0 = false, 1 = true
});
...
export const feedPostsTable = sqliteTable("feed_posts", {
    ...
    submittedAt: int('submitted_at', { mode: 'timestamp' }).notNull(), // sqlite는 timestamp 타입이 없으므로 int로 저장 // 원본 post의 createdAt과는 별개로 현재 owner가 언제 추가했는지를 기록
}, (table) => [
...
export const feedIngestionSourcesTable = sqliteTable("feed_ingestion_sources", {
    ...
    title: text('title'), // 소스 제목 (optional)
    description: text('description'), // 소스 설명 (optional)
}, (table) => [
